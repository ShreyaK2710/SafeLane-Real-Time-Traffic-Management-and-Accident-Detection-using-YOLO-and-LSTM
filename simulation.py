...
signals = []
noOfSignals = 4
simTime = 480       # change this to change time of simulation
timeElapsed = 0
...
def generateVehicles():
    while True:
        vehicle_type = random.randint(0, 4)  # 0 to 3 -> normal vehicles, 4 -> ambulance

        # [NEW CODE] Increase probability of ambulance (10% chance)
        is_ambulance = random.random() < 0.1  # 10% probability an ambulance appears
        if is_ambulance:
            vehicle_type = 4  # Assign as an ambulance
            lane_number = 0  # Always give the ambulance priority lane
        else:
            if vehicle_type == 4:  
                lane_number = 0  # Assign priority lane for ambulance
            else:
                lane_number = random.randint(0, 1) + 1  # Other vehicles choose between lanes 1 and 2
        ...
        # [NEW CODE] Mark ambulance waiting in the system
        if vehicle_type == 4:
            vehicles[directionNumbers[direction_number]]['waiting_ambulance'] += 1
...
# Vehicle types
vehicleTypes = ["car", "truck", "bus", "bike", "ambulance"]

directionNumbers = ["right", "down", "left", "up"]
vehicles = {
    "right": {"crossed": 0, "waiting_ambulance": 0},
    "down": {"crossed": 0, "waiting_ambulance": 0},
    "left": {"crossed": 0, "waiting_ambulance": 0},
    "up": {"crossed": 0, "waiting_ambulance": 0},
}
...
class Vehicle:
    def __init__(self, lane, vehicle_type, direction, direction_name, will_turn):
        self.lane = lane
        ...
        self.currentImage = pygame.image.load(f'images/{vehicle_type}.png')
...
def generateVehicles():
    while True:
        ...
        # Increase probability of ambulance (10% chance)
        is_ambulance = random.random() < 0.1  # 10% probability an ambulance appears
        if is_ambulance:
            vehicle_type = 4  # Assign as an ambulance
            lane_number = 0  # Always give the ambulance priority lane
        ...
        # Mark ambulance waiting in the system
        if vehicle_type == 4:
            vehicles[directionNumbers[direction_number]]['waiting_ambulance'] += 1
...
def simulationTime():
    ...
    while True:
        ...
        time.sleep(1)
        if timeElapsed == simTime:
            totalVehicles = sum(vehicles[d]['crossed'] for d in directionNumbers)
...
class Main:
    ...
    while True:
        ...
        for i in range(4):  # Display signals
            if vehicles[directionNumbers[i]]['waiting_ambulance'] > 0:
                signals[i].signalText = "PRIORITY"
                screen.blit(greenSignal, signalCoods[i])
            else:
                screen.blit(redSignal, signalCoods[i])
